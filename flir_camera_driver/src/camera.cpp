...
void Camera::setNewConfiguration(FlirConfig& config, const uint32_t& level)
{
try
  {
    ...
    {
      setProperty(node_map_, "AutoExposureExposureTimeUpperLimit", static_cast<float>(config.auto_exposure_time_upper_limit));
    }
...
void Camera::setImageControlFormats(flir_camera_driver::FlirConfig &config)
{
